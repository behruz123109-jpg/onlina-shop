...
async def init_db():
    ...
    async with pool.acquire() as db:
        ...
        prod_exists = await db.fetchrow("SELECT id FROM products LIMIT 1")
        if not prod_exists:
            await db.executemany(
                "INSERT INTO products(category_id,name,description,price,stock) VALUES($1,$2,$3,$4,$5)", [
                    (1, 'Olma', 'Yangi, mazali, tabiiy olma.', 5000, 120),
                    (1, 'Banan', 'Sog‘lom va shirin.', 7000, 80),
                    (2, 'Sut', '100% tabiiy sut, yangi.', 12000, 50),
                    (2, 'Gazlangan ichimlik', 'Sovutgichdan yangi.', 9000, 60),
                    (3, 'Non', 'Haqiqiy non, toza ingredientlardan.', 2500, 120),
                    (3, 'Bulochka', 'Shirin bulochka, ertalab uchun mukammal.', 4500, 90),
                    (4, 'Telefon qopqog‘i', 'Chiroyli, bardoshli himoya qopqoq.', 30000, 25)
                ]
            )
...
async def main():
    ...
    # 🌐 1. WEB API ni yoqamiz (8080 portda sayt uchun xizmat qiladi)
    app = web.Application()
    ...
    await site.start()
    print("🌐 Web API Server [PORT: 8080] da muvaffaqiyatli ishga tushdi...")
